...
public:
  ...
  ///Converts this matrix into a pure scaling matrix.
  static Matrix4 Scale(Vec3Param axis);
  ...
  ///Converts this matrix into a pure translation matrix for 3-D vectors.
  static Matrix4 Translate(Vec3Param axis);
  ...
  static Matrix4 PerspectiveProjection(float FOV, int width, int height, float zNear, float zFar);

  static Matrix4 LookAt(const Vector3& vEye, const Vector3& vTarget, const Vector3& vInUp);

  static Matrix4 RotateEuler(float a, float b, float c);//z-x-y convention
...
Matrix4 operator*(Mat4Param lhs, Vec4Param rhs);
