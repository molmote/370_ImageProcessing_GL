...
vec2 calculateUV()
{
	...
	else if (projectorType == CUBIC)
	{
		vec3 vtxVec = normalize(smoothVertex.xyz);
		vec3 xAxis = vec3(1, 0, 0);
		float x_dot_v = dot(xAxis, vtxVec);

		vec3 yAxis = vec3(0, 1, 0);
		vec3 zAxis = vec3(1, 0, 1);
	}
	return uv;
}
...
vec4 computeLightingTerm(in int lightIdx, in vec4 n, in vec4 vtx, in vec4 materialDiffuse, in vec4 materialSpecular)
{
	...
	else
	{
		...
		vec4 h = normalize((v + l)*0.5);
		specular = light.specular * materialSpecular * pow(max(dot(n, h), 0), Material.shininess);
		if (light.type == SPOT)
		{
